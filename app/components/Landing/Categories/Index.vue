...
<template>
  <UContainer v-if="props.categories.length" class="py-8">
    ...
    <div class="bg-white rounded-lg border border-neutral-200 shadow-sm p-6 px-10">
      <UCarousel
        v-slot="{ item }"
        ...
        :ui="{
          item: 'basis-1/5',
          container: 'gap-6',
          prev: 'sm:-start-10 hover:bg-transparent cursor-pointer',
          next: 'sm:-end-10 hover:bg-transparent cursor-pointer',
        }">
        <NuxtLink
          :to="`/category/${item.slug}`"
          class="flex flex-col items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity">
          <div class="w-32 h-32 bg-neutral-100 rounded-md flex items-center justify-center overflow-hidden">
